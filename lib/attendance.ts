...
function parseVerifyResult(data: unknown): AttendanceVerifyResult {
  if (!data || typeof data !== 'object') return { valid: false, type: 'ATTENDANCE' }
  const o = data as Record<string, unknown>
  if (o.valid !== true) return { valid: false, type: 'ATTENDANCE' }
  const recipientRaw = o.recipient
  ...
  return {
    valid: true,
    type: typeof o.type === 'string' ? o.type : 'ATTENDANCE',
...
export async function claimAttendanceCertificate(email: string): Promise<AttendanceClaimResult> {
  const normalized = email.trim().toLowerCase()
  const res = await fetch(`${ATTENDANCE_API}/claim`, {
    method: 'POST',
